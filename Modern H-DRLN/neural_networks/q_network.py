...
class QNetwork(nn.Module):
    def __init__(self, input_dimension=3*64, hiddenWidth1=100, hiddenWidth2=64, outputWidth=5, weightInit=-1):
        super(QNetwork, self).__init__()

        # Hidden Layer 1
        self.W_hidden1 = nn.Parameter(
            truncated_normal.truncated_normal(torch.ones((input_dimension, hiddenWidth1)), std=0.1), requires_grad=True)
    ...
    def forward(self, x):
        print(x.shape)
        print(self.W_hidden1.shape)
        print(self.b_hidden1.shape)
